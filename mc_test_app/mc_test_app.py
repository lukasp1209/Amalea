...
def main():
    ...
    if st.session_state.get("test_time_expired", False):
        ...
        # --- NEU: Nach Review-Sprung Erklärung für Ziel-Frage aktivieren ---
        if next_idx is None and st.session_state.get("_force_show_explanation", False):
            # Suche die Frage, für die show_explanation_X True ist
            for idx in indices:
                if st.session_state.get(f"show_explanation_{idx}", False):
                    next_idx = idx
                    break
            st.session_state["_force_show_explanation"] = False
